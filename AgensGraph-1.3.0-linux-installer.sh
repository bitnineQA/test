...
# port check
_port_check() {
	nstat=`netstat -ant4 | grep LISTEN | grep $agport | awk '{print $4}' | sed 's/\:/ /g' | awk '{print $2}'`
	if [ "$nstat" = "" ]; then
...
# set env
mv $HOME/.bash_profile.agens $HOME/.bash_profile.agens.bk
_create_env

#binary mv
tar xzf AgensGraph_v1.3.0_linux.tar.gz
mv AgensGraph $instdir
rm AgensGraph_v1.3.0_linux.tar.gz
...
echo
echo "1) set env (. ~/.bash_profile.agens)"
